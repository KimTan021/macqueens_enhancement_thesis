...
from utils.visualize_kmeans import visualize_kmeans
from utils.print_cluster_data import print_cluster_data
from utils.animate_outliers import IsolationForestVisualizer
...
# Extract the "LAN" and "LON" columns for visualization
data = df[["LAT", "LON"]].values

# Set up the animation
fig, ax = plt.subplots()
visualizer = IsolationForestVisualizer(data, contamination=0.1, ax=ax, anim=None)
anim = FuncAnimation(fig, visualizer.fit, frames=len(data), interval=500, repeat=False)
...
optimal_k, labels, outliers = find_optimal_clusters(df[["AREA CODE"]], plot=True, detect_outliers=detect_outliers, plot_outliers=True)
...
# visualize_kmeans(df, n_clusters=len(df['cluster'].unique()), algorithm="macqueen", init="adaptive", max_iter=300, noise_level=noise_level)


# Ensure the 'cluster' column exists in the DataFrame
# if "cluster" in df.columns:
#     # Print each cluster's data
#     print_cluster_data(df)
# else:
